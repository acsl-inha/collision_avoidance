...
class AircraftEnv(gym.Env):
    ...
    def __init__(self, succeed_coef = 8000, collide_coef = -2000, change_cmd_penalty = -200, cmd_penalty = -1, start_cond_coef = 90, cmd_suit_coef = 0.01):
        ...
        self.dist_sep = 110                     # near mid-air collision range
    ...
    def step(self, action):
        ...
        if self.r <= self.dist_sep:
            if self.r <= self.start_cond_coef:
                reward = self.collide_coef
            else:
                reward = (self.succeed_coef - self.collide_coef) / 2 * np.sin(np.pi / (self.dist_sep - self.start_cond_coef) * (self.r - (self.dist_sep + self.start_cond_coef) / 2)) \
        + (self.succeed_coef + self.collide_coef) / 2
            done = True
    ...
    def reset(self):
        self.__init__(self.succeed_coef, self.collide_coef, self.change_cmd_penalty, self.cmd_penalty, self.start_cond_coef, self.cmd_suit_coef)
        return self._state
