...
class AircraftEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.r<=self.dist_sep:
            ...
            if self.h_cmd_count==0:
                reward=int(0.1*(self.r-200)**2+1000)
            else:
                reward=int(0.4*(self.r-200)**2+1000)
            done=True
        ...
        if not done:
            ...
            self.azim_p = self.azim
            self.daz_p = self.daz
            self._state=np.array([self.r,self.vc,self.los,self.daz,self.dlos])/self.high
            
            self.t_step+=1
            
            reward=np.abs(self.hdot_cmd)*(-0.01)*self.t_step
