...
class AircraftEnv(gym.Env):
    ...
    def __init__(self, succeed_coef = 8000, change_cmd_penalty = -200, cmd_penalty = -1, start_cond_coef = 1):
    ...
    def step(self, action):
        ...
        if self.r <= self.dist_sep:
            reward = int((10000 * 0.02 * (10000 * np.log(( - self.r + 110) / 10 * np.exp(1)) / (10 * ( - self.r + 110)) - 10000 * 0.009) - 2000) + self.succeed_coef * (100- self.start_cond_coef * abs(self.height_diff)) * 0.01)
            done = True
        ...
        if not done:
            ...
            reward += np.abs(self.hdot_cmd)*self.cmd_penalty*self.t_step

        return self._state.flatten(), reward, done, {"info": [self.hdot_cmd, self.r, self.elev, self.azim, self.Pm_NED, self.Pt_NED, self.h]}

    def reset(self):
        self.__init__(self.succeed_coef, self.change_cmd_penalty, self.cmd_penalty, self.start_cond_coef)
        return self._state
